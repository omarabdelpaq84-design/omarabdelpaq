...
// Build the "professional skills" progress-circle dots (structure only —
// they get lit up once the Skills section scrolls into view, below).
...
// Scroll-triggered Skills animation: bars fill, circles light up, and the
// percentage numbers count up — all only once, the first time the Skills
// section enters the viewport (instead of firing instantly on page load,
// which meant it had already finished by the time anyone scrolled to it).
...
function animateCircle(elem) {
    ...
    for (let i = 0; i < percent; i++) {
        pointsMarked[i].style.animationDelay = (i * 0.03) + 's';
        pointsMarked[i].classList.add('marked');
    }
}

function animateCountUp(elem) {
    var target = parseInt(elem.getAttribute('data-count'), 10) || 0;
    var duration = 1600;
    var startTime = null;
    ...
    function step(timestamp) {
        ...
        elem.textContent = Math.floor(eased * target) + '%';
        if (progress < 1) {
